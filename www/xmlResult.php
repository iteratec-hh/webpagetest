...
if (isset ( $test ['test'] ) && $test ['test'] ['batch'])
	...
	if (isset ( $test ['test'] ['completeTime'] )) {
		...
		$pageDataArray = loadAllPageData ( $testPath, array (
				'allEvents' => true 
		) );
		...
		echo "<average>\n";
		echo "<firstView>\n";
		foreach ( array_keys ( $pageDataArray ) as $eventName ) {
			echo "<testStep>";
			foreach ( $fvArray [$eventName] as $key => $val ) {
				$value = is_numeric ( $val ) ? number_format ( $val, 0, '.', '' ) : $val;
				echo "<$key>" . $value . "</$key>\n";
			}
			echo "</testStep>\n";
		}
		...
		if (! empty ( $rvArray )) {
			...
			foreach ( array_keys ( $pageDataArray ) as $eventName ) {
				echo "<testStep>";
				foreach ( $rvArray [$eventName] as $key => $val ) {
					$value = is_numeric ( $val ) ? number_format ( $val, 0, '.', '' ) : $val;
					echo "<$key>" . $value . "</$key>\n";
				}
				echo "</testStep>";
		...
		$fvMedianArray = array ();
		foreach ( $pageDataArray as $eventName => $pageData ) {
			$fvMedianArray [$eventName] = GetMedianRun ( $pageData, 0, $median_metric );
		}
		
		if (! empty ( $fvMedianArray )) {
			echo "<median>\n";
			echo "<firstView>\n";
			
			foreach ( $pageDataArray as $eventName => $pageData ) {
				$fvMedian = $fvMedianArray [$eventName];
			...
			if (! empty ( $rvArray )) {
				$rvMedianArray = array ();
				foreach ( $pageDataArray as $eventName => $pageData ) {
					$rvMedianArray [$eventName] = GetMedianRun ( $pageData, 1, $median_metric );
				}
				if (! empty ( $rvMedianArray )) {
					echo "<repeatView>\n";
					foreach ( $pageDataArray as $eventName => $pageData ) {
						$rvMedian = $rvMedianArray [$eventName];
			...
			if (isset ( $pageData [$i] )) {
				if (isset ( $pageData [$i] [0] )) {
					...
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						...
						echo "http://$host$uri/result/$id/{$i}/" . urlencode ( $eventName ) . "/waterfall_thumb.png";
						echo "</testStep>";
					}
					echo "</waterfall>\n";
					echo "<checklist>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri/result/$id/{$i}/" . urlencode ( $eventName ) . "/optimization_thumb.png";
						echo "</testStep>";
					}
					echo "</checklist>\n";
					echo "<screenShot>";
					$pageRunData = loadPageRunData ( $testPath, $i, 0, array (
							'allEvents' => true 
					) );
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri/result/$id/{$i}_" . $pageRunData [$eventName] ["eventNumber"] . "_screen_thumb.jpg";
						echo "</testStep>";
					}
					echo "</screenShot>\n";
					echo "</thumbnails>\n";
					...
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_" . urlencode ( $eventName ) . "_waterfall.png";
						echo "</testStep>";
					}
					echo "</waterfall>\n";
					echo "<connectionView>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_" . urlencode ( $eventName ) . "_connection.png";
						echo "</testStep>";
					}
					echo "</connectionView>\n";
					echo "<checklist>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_" . urlencode ( $eventName ) . "_optimization.png";
						echo "</testStep>";
					}
					echo "</checklist>\n";
					echo "<screenShot>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_" . $pageRunData [$eventName] ["eventNumber"] . "_screen.jpg";
						echo "</testStep>";
					}
					echo "</screenShot>\n";
					...
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_" . $pageRunData [$eventName] ["eventNumber"] . "_report.txt";
						echo "</testStep>";
					}
					echo "</headers>\n";
					...
					if (array_key_exists ( 'frames', $progress ) && is_array ( $progress ['frames'] ) && count ( $progress ['frames'] )) {
						...
						foreach ( array_keys ( $pageDataArray ) as $eventName ) {
							echo "<testStep>";
							echo "<eventName>" . $eventName . "</eventName>";
							$eventNumber = $pageRunData [$eventName] ["eventNumber"];
							$options = array("eventNumber" => $eventNumber);
							$startOffset = array_key_exists ( 'testStartOffset', $pageRunData [$eventName] ) ? intval ( round ( $pageRunData [$eventName] ['testStartOffset'] ) ) : 0;
							$progress = GetVisualProgress ( $testPath, $i, 0, $options, null, $startOffset);
							if (array_key_exists ( 'frames', $progress ) && is_array ( $progress ['frames'] ) && count ( $progress ['frames'] )) {							
								foreach ( $progress ['frames'] as $ms => $frame ) {
									echo "<frame>\n";
									echo "<time>$ms</time>\n";
									echo "<image>http://$host$uri$path/video_{$i}_{$eventNumber}/{$frame['file']}</image>\n";
					...
					if (isset ( $progress ) && is_array ( $progress ) && array_key_exists ( 'DevTools', $progress ) && is_array ( $progress ['DevTools'] )) {
				...
				if (isset ( $pageData [$i] [1] )) {
					...
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri/result/$id/{$i}/" . urlencode ( $eventName ) . "/Cached_waterfall_thumb.png";
						echo "</testStep>";
					}
					echo "</waterfall>\n";
					echo "<checklist>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri/result/$id/{$i}/" . urlencode ( $eventName ) . "/Cached_optimization_thumb.png";
						echo "</testStep>";
					}
					echo "</checklist>\n";
					$pageRunDataCached = loadPageRunData ( $testPath, $i, 1, array (
							'allEvents' => true 
					) );
					echo "<screenShot>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri/result/$id/{$i}_Cached_" . $pageRunDataCached [$eventName] ["eventNumber"] . "_screen_thumb.jpg";
						echo "</testStep>";
					}
					echo "</screenShot>\n";
					echo "</thumbnails>\n";
					...
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_Cached_" . urlencode ( $eventName ) . "_waterfall.png";
						echo "</testStep>";
					}
					echo "</waterfall>\n";
					echo "<connectionView>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_Cached_" . urlencode ( $eventName ) . "_connection.png";
						echo "</testStep>";
					}
					echo "</connectionView>\n";
					echo "<checklist>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_Cached_" . urlencode ( $eventName ) . "_optimization.png";
						echo "</testStep>";
					}
					echo "</checklist>\n";
					echo "<screenShot>";
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_Cached_" . $pageRunDataCached [$eventName] ["eventNumber"] . "_screen.jpg";
						echo "</testStep>";
					}
					echo "</screenShot>\n";
					...
					foreach ( array_keys ( $pageDataArray ) as $eventName ) {
						echo "<testStep>";
						echo "<eventName>" . $eventName . "</eventName>";
						echo "http://$host$uri$path/{$i}_Cached_" . $pageRunDataCached [$eventName] ["eventNumber"] . "_report.txt";
						echo "</testStep>";
					}
					echo "</headers>\n";
					...
					if (array_key_exists ( 'frames', $progress ) && is_array ( $progress ['frames'] ) && count ( $progress ['frames'] )) {
						...
						foreach ( array_keys ( $pageDataArray ) as $eventName ) {
							...
							$options = array("eventNumber" => $eventNumber);
							$startOffset = array_key_exists ( 'testStartOffset', $pageRunData [$eventName] ) ? intval ( round ( $pageRunData [$eventName] ['testStartOffset'] ) ) : 0;
							$progress = GetVisualProgress ( $testPath, $i, 1, $options, null, $startOffset);
							if (array_key_exists ( 'frames', $progress ) && is_array ( $progress ['frames'] ) && count ( $progress ['frames'] )) {
								foreach ( $progress ['frames'] as $ms => $frame ) {
									echo "<frame>\n";
									echo "<time>$ms</time>\n";
									echo "<image>http://$host$uri$path/video_{$i}_{$eventNumber}_cached/{$frame['file']}</image>\n";
									echo "<VisuallyComplete>{$frame['progress']}</VisuallyComplete>\n";
									echo "</frame>\n";
								}
							}
